...
public class CS400Graph<NodeType,EdgeType extends Number> implements GraphADT<NodeType,EdgeType> {
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path copyPath, Edge extendBy) {
            ...
            this.distance = copyPath.distance + extendBy.weight.doubleValue(); // Adds the cost of the edge
            this.dataSequence = new LinkedList<>(); // Constructing the new instance dataSequence field
            for (NodeType x : copyPath.dataSequence) {
    ...
    protected Path dijkstrasShortestPath(NodeType start, NodeType end) {
        ...
        if(vertices.get(start) == null || vertices.get(start) == null) {throw new NoSuchElementException();} 

        Vertex startNode = vertices.get(start);
        Path path = new Path(startNode); 
        PriorityQueue<Path> frontier = new PriorityQueue<>(); // initialize the priority queue
        frontier.add(path); // addds the first path

        // keep loop until it reaches to the end or priority is empty
        // or went through all the nodes
        while (!path.end.data.equals(end) && !frontier.isEmpty()) {
            path = frontier.poll();
            for (Edge e : path.end.edgesLeaving) {
                Path p = new Path(path,e);
                frontier.add(p);
            }
        }
        if (path.end.data.equals(end)) {return path;}
        else throw new NoSuchElementException(); // when fails to find any path from the start to end vertices
    }
